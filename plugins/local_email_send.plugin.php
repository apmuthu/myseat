...
Description: This plugin sends a email confirmation + attachments after booking
Version: 1.0
...
function my_email_send_conf() {
	...
	// *** send confirmation email
			...
			//switch LOCAL/SMTP/sendmail  email use (see the config.general.php file)
			if($settings['emailSMTP'] != 'LOCAL'){
				...
				{
						// works better when auth isn't required
						// allows trivial sending of attachments
          					$mail->IsSendmail();
        			}
...
//
         			$mail->AddAttachment('/data/MENU1.pdf');      // attachment
         			$mail->AddAttachment('/data/THE_BAR_MENU.pdf'); // attachment
         			$mail->AddAttachment('/data/Cakes.pdf'); // attachment
